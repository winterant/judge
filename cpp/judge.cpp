...
int compile()
{
    if(solution.language==3)//python不需要编译
        return 0;
    //编译用户提交的代码
    const char *CP_C[]  ={"gcc","Main.c",  "-o","Main","-Wall","-lm","--static","-std=c17",  "-fmax-errors=5","-DONLINE_JUDGE","-O2",NULL};
    const char *CP_CPP[]={"g++","Main.cpp","-o","Main","-Wall","-lm","--static","-std=c++17","-fmax-errors=5","-DONLINE_JUDGE","-O2","-fno-asm", NULL};
...
// 编译spj.cpp
int compile_spj(char *spj_path)
{
    const char *CP_CPP[]={"g++",spj_path,"-o","spj","-O2","-lm","-std=c++17", NULL};
    int pid;
    if( (pid=fork()) == 0 ) //子进程编译
    {
        struct rlimit LIM;
        int cpu_time = 600;     //60s, compile time limit

        LIM.rlim_max=LIM.rlim_cur=cpu_time;
        setrlimit(RLIMIT_CPU, &LIM);

        LIM.rlim_max=LIM.rlim_cur=(1024<<20);//file size limit: 1024MB,compile file size limit
        setrlimit(RLIMIT_FSIZE, &LIM);
    ...
    else if(pid>0) //父进程
    {
        ...
        {
            char error[128];
            sprintf(error,"[ERROR]: spj.cpp compile error. exit code is %d(0x%08x)\n",status,status);
            write_file(error,"ce.txt","a+");
            print_log("%s",error);
        }
        return status;   //+:compile error
...
//运行special judge
int running_spj(const char *data_in_path,const char *data_out_path,const char *user_out_path,const char *test_name)
{
    struct rlimit LIM;
    //time limit
    ...
    int ret = system_cmd("./spj %s %s %s > spj_temp.out",data_in_path,data_out_path,user_out_path);
...
int main (int argc, char* argv[])
{
    ...
    else    //编译成功，运行
    {
        ...
        sprintf(spj_path,"%s/%d/spj/spj.cpp",JG_DATA_DIR,solution.problem_id); //特判程序spj的路径
